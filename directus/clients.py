...
class DirectusClient_V9():
    ...
    def login(self, email: str = None, password: str = None) -> tuple:
        '''
        Login with the /auth/login endpoint. Returns both the access token and refresh token. Updates self.email and self.password if email and passwords are provided.
        '''
    ...
    def logout(self, refresh_token: str = None) -> None:
        '''
        Logout with the /auth/logout endpoint.
        '''
    ...
    def get_token(self):
        '''
        Returns static token if there is any, if not refreshes the temporary token.
        '''
    ...
    def bulk_insert(
        ...
        for i in range(0, length, interval):
            if verbose:
                print(
                    f"Inserting record {i}-{min(i+100, length)} out of {length}..."
                )
            self.post(f"/items/{collection_name}", json=items[i:i + interval])
        print(f"Records successfully created!")
    ...
    def duplicate_collection(
        ...
        self.post(f"/collections", json=duplicate_collection)
        fields = [
            field for field in self.get_all_fields(collection_name)
            if not field['schema']['is_primary_key']
        ]
        for field in fields:
            print(f'Creating fields...')
            self.post(f"/fields/{duplicate_collection_name}", json=field)
        print(f'Fields successfully created!')
        self.bulk_insert(
            duplicate_collection_name,
            self.get(f'/items/{collection_name}', params={"limit": -1})
        )

    # Checks if collection exists in directus
    def collection_exists(self, collection_name: str) -> bool:
        collection_schema = [
            col['collection'] for col in self.get('/collections')
        ]
    ...
    def delete_all_items(self, collection_name: str) -> None:
        ...
        item_ids = [
            data['id'] for data in self.get(
                f"/items/{collection_name}?fields={pk_name}",
                params={"limit": -1}
            )
        ]
        if len(item_ids) == 0:
    ...
    def get_all_fields(self, collection_name: str) -> list:
        '''
        Return all fields in the directus collection. Remove the id key in meta to avoid errors in inserting this directus field again.
        '''
        fields = self.get(f"/fields/{collection_name}")
        for field in fields:
            if 'meta' in field and field['meta'] is not None and 'id' in field[
                'meta'
            ]:
                field['meta'].pop('id')
    ...
    def get_pk_field(self, collection_name: str) -> dict:
        ...
        '''
        return [
            field for field in self.get(f'/fields/{collection_name}')
    ...
    def get_all_fk_fields(self, collection_name: str) -> dict:
        '''
        Return all foreign keys in the directus collection
        '''
        return [
            field for field in self.get(f"/fields/{collection_name}")
