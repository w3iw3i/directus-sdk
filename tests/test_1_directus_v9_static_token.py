...
@pytest.mark.parametrize(
    ...
    [("test_collection", "int"), ("test_relations_collection", "int"),
...
def test_create_field(collection_name: str, field_name: str, field_type: str):
    ...
    assert resp['data']['field'] == field_name
    assert resp['data']['type'] == field_type
...
def test_create_items(collection_name: str, name: str, comments: str):
    resp = client.post(
        f"/items/{collection_name}", json={
            "name": name,
            "comments": comments
        }
    )
    assert resp['data']["name"] == name
    assert resp['data']["comments"] == comments


@pytest.mark.parametrize("collection_name", [("test_collection")])
def test_delete_item(collection_name: str):
    resp = client.post(
        f"/items/{collection_name}", json={
            "name": "test",
            "comments": "test"
    ...
    client.delete(f"/items/{collection_name}/{resp['data']['id']}")
    with pytest.raises(AssertionError):
        client.get(f"/items/{collection_name}/{resp['data']['id']}")
...
def test_update_item(collection_name: str, id: int, name: str, comments: str):
    resp = client.patch(
...
@pytest.mark.parametrize(
    "collection_name, num_of_items", [("test_collection", 100)]
)
def test_bulk_insert(collection_name: str, num_of_items: int):
    ...
    assert len(client.get(f"/items/{collection_name}")) == num_of_items
...
def test_duplicate_collection(
    original_collection_name: str, duplicated_collection_name: str
):
    client.duplicate_collection(
        original_collection_name, duplicated_collection_name
    )
    assert client.get(f"/collections/{duplicated_collection_name}"
                      )["collection"] == duplicated_collection_name


@pytest.mark.parametrize(
    "collection_name, exists",
    [("test_collection", True), ("duplicated_collection", True),
     ("no_collection", False)]
)
...
@pytest.mark.parametrize("collection_name", [("duplicated_collection")])
def test_delete_all_items(collection_name: str):
    client.delete_all_items(collection_name)
    assert client.get(f'/items/{collection_name}') == []


@pytest.mark.parametrize(
    "collection_name, data_type",
    [("test_collection", "integer"), ("duplicated_collection", "integer"),
     ("uuid_pk_collection", "uuid")]
)
def test_integer_pk(collection_name: str, data_type: str):
    assert client.get_pk_field(collection_name)['type'] == data_type


@pytest.mark.parametrize(
    "collection_names", [([
        'test_collection', 'duplicated_collection', 'uuid_pk_collection',
        'test_relations_collection'
    ])]
)
def test_get_all_user_created_collection_names(collection_names: list):
    assert sorted(client.get_all_user_created_collection_names()
                  ) == sorted(collection_names)
...
def test_post_relations(
    collection_name: str, field: str, related_collection: str
):
    ...
    assert client.get(f"/relations/{collection_name}/{field}"
                      )['related_collection'] == related_collection


@pytest.mark.parametrize(
    "collection_name, fk_field_names",
    [("test_relations_collection", ["uuid_fk"]), ("test_collection", []),
     ("duplicated_collection", []), ("uuid_pk_collection", [])]
)
def test_get_fk_fields(collection_name: str, fk_field_names: list):
    fk_fields = [
...
@pytest.mark.parametrize(
    "collection_name", [("test_collection"), ("duplicated_collection"),
                        ("test_relations_collection"), ("uuid_pk_collection")]
)
